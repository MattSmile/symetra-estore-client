    ...
    switch (type) {
      case 'all':
        return (
          ...
          encodeURIComponent(JSON.stringify(state.purchases))
        );
      case 'coupon':
        return (
          'data:text/json;charset=utf-8,' +
          encodeURIComponent(
            JSON.stringify(
              state.purchases.filter((purchase) => purchase.couponApplied)
            )
          )
        );
      default:
    ...
    <div className="Admin">
      <section>
        ...
        <form id="interval_form" onSubmit={submitCouponInterval}>
          ...
          <input
            ...
            type="number"
            id="coupon_interval"
            defaultValue={couponInterval}
            onChange={(e) =>
              onChangeCouponInterval(parseInt(e.currentTarget.value))
            }
          />
      ...
      <section>
        ...
        <div className="Reports">
          <a href={generateReport('all')} download={'report.json'}>
            Download All Purchases
          </a>
          <a href={generateReport('coupon')} download={'coupon-report.json'}>
            Download Coupon Purchases
          </a>
        </div>
      </section>
